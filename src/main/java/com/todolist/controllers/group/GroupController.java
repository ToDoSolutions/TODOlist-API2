...
public class GroupController {
    ...
    public List<Map<String, Object>> getAllGroups(@RequestParam(defaultValue = "0") @Min(value = 0, message = "The offset must be positive.") Integer offset,
        ...
        return result.stream().map(group -> new ShowGroup(group, groupUserService.getShowUsersFromGroup(group))).map(group -> dtoManager.getEntityAsJson(group,
    ...
    public ResponseEntity<ShowGroup> addGroup(@RequestBody @Valid Group group) {
