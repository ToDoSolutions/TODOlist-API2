...
class PutTest {
    ...
    public void setUp() {
        ...
        showTask = restTemplate.postForObject(uri + "/task", task, ShowTask.class);
    ...
    void testPutFine() {
        ...
        task.setIdTask(showTask.getIdTask());
        ShowTask response = restTemplate.exchange(uri + "/task", HttpMethod.PUT, new HttpEntity<>(task), ShowTask.class).getBody();
    ...
    void testPutNotExists() {
        task = Task.of("Task 2", "Task 2 description", "Task 2 annotation", "DONE",
                "3000-01-22", "2015-01-01", 1L, "EASY");
        task.setIdTask(99L);
        ManagerException.of(assertThrows(HttpClientErrorException.class, () -> restTemplate.exchange(uri + "/task", HttpMethod.PUT, new HttpEntity<>(task), ShowTask.class)))
    ...
    void testPutWithNullOrEmptyTitle() {
        ...
        task.setTitle("");
        response = restTemplate.exchange(uri + "/task", HttpMethod.PUT, new HttpEntity<>(task), ShowTask.class).getBody();
